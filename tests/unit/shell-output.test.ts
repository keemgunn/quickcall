...
const emptyConfig = (over: Partial<Config> = {}): Config => ({
  tool: { tools: {} },
  ...over,
});

async function shellContext() {
  const cwd = await tempDir("shell-output");
  clean.push(cwd);
...
it("executes substitutions concurrently and replaces in source order", async () => {
  ...
  const result = await expandShell(
...
it("validates the selected shell even when no substitutions exist", async () => {
  const { cwd, environment } = await shellContext();
  await expect(expandShell("plain", "/definitely/missing-shell", emptyConfig(), cwd, environment)).rejects.toThrow(
    "shell executable",
  );
  await expect(expandShell("plain", "/", emptyConfig(), cwd, environment)).rejects.toThrow("shell executable");
});

it("executes arithmetic expansion when permissions are unconfigured", async () => {
  const { cwd, environment } = await shellContext();
  await expect(expandShell("!`echo $((1 + 2))`", "/bin/sh", emptyConfig(), cwd, environment)).resolves.toBe("3\n");
});

it("discovers only complete exact expressions, expands concurrently, and does not recurse", async () => {
  const { cwd, environment } = await shellContext();
  expect(expressions("x !`printf one` !`printf two` !`open")).toHaveLength(2);
  // No permission preflight — both expressions expand even without a matching allow rule.
  await expect(
    expandShell("!`printf first` !`printf second`", "/bin/sh", emptyConfig(), cwd, environment),
  ).resolves.toBe("first second");
  await expect(expandShell("!`printf '\\041\\140nested\\140'`", "/bin/sh", emptyConfig(), cwd, environment)).resolves.toBe(
    "!`nested`",
  );
});
...
it("keeps stdout from failed commands, drains stderr, and runs in one concurrent batch", async () => {
  ...
  const output = await expandShell(
