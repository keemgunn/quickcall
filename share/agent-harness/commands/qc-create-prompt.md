...
Create one reusable qc-compatible Markdown prompt. Write the file; do not merely return draft text.
...
- Settings: only explicit model, thinking, skill loading, or approval behavior that must differ from defaults.

Do not ask for optional fields the prompt does not need. Do not invent model IDs, thinking values, skill paths, approval policy, or shell commands.

## Choose Location

| Intent | File path | Invocation form |
| --- | --- | --- |
| Reusable only in the current project | `<project>/.qc/prompts/<alias>.md` | Run from that project: `qc <alias>` |
| Reusable across projects for this user | `~/.qc/prompts/<alias>.md` | `qc <alias>` when no cwd project alias shadows it |
| Explicit path or prompt outside alias stores | User-selected path ending `.md` | `qc <direct-path>.md` |
...
qc_no_skills: <true or false only when required>
qc_skill_path: <one path only when required>
qc_approve: <true or false only when required>
---
```

Delete every unused line from the template. Never emit empty settings or use alternative field prefixes, shell settings, arbitrary Pi flags, `$1`, or `$ARGUMENTS`.
...
- Explain that commands run in the qc invocation cwd with inherited environment.
- Check expected global/project `[command-permissions]` when available; do not weaken rules to make the prompt pass.

Remember that the caller's future `--append` text can contain the same executable syntax. Project `.qc/config.toml` is trusted executable configuration because it can select the shell and apply later permission rules.
...
6. Construct a shell-quoted invocation example with only needed options.

Do not execute qc merely to validate it: qc launches Pi and may execute shell expressions. Run it only when the user explicitly asks to execute the new prompt.
...
- Exact invocation example and required cwd when relevant.
- Included `qc_*` settings (including `qc_cli` when set).
- Shell expressions and permission/trust findings, or `none`.
